...
public interface Edk extends Library {
	 ...
	 *  \param eegRate      - If 0, EEG controller rate is 128Hz.
	 *                      - If 1, EEG controller rate is 256Hz.
	 *                      - If 2, no signals.
	 *  \param eegRes       - If 0, EEG Resolution is 14bit.
	 *                      - If 1, EEG Resolution is 16bit.
	 *                      - If 2, no signals.
	 *  \param memsRate     - If 0, MEMS controller rate is OFF.
	 *                      - If 1, MEMS controller rate is 32Hz.
	 *                      - If 2, MEMS controller rate is 64Hz.
	 *                      - If 3, MEMS controller rate is 128Hz.
	...
	*/
	 ...
	 *  \param eegRate      - If 0, then EEG controller rate is 128Hz.
	 *                      - If 1, then EEG controller rate is 256Hz.
	 *  \param eegRes       - If 0, then EEG Resolution is 14bit.
	 *                      - If 1, then EEG Resolution is 16bit.
	 *  \param memsRate     - If 0, then MEMS controller rate is OFF.
	 *                      - If 1, then MEMS controller rate is 32Hz.
	 *                      - If 2, then MEMS controller rate is 64Hz.
	 *                      - If 3, then MEMS controller rate is 128Hz.
	...
	*/
    ...
    //! Return number of controller of motion data stored in the data handle
    /*!
     *  \param hData - a handle returned by IEE_MotionDataCreate()
     *  \param nSampleOut - receives the number of controller of data stored in the data handle
     *  \return EDK_ERROR_CODE
     *                 - EDK_OK if successful
     */
